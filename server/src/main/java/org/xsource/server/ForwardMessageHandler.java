    ...
    String process() {
        ...
        response.put("type","forward_response");
            if(jsonObject.getString("sub_type").equals("forward")){
                ...
                try (var printer = new PrintWriter(Server.getInstance().getClients().get(toUser).getOutputStream(), true)){
                    if(Server.getInstance().getClients().containsKey(toUser)) {
                        printer.println(forward_msg.toString());
